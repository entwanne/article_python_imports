...
[^callables]: Ou objets assimilés, voir [le chapitre dédié du cours Notion de Python avancées](https://zestedesavoir.com/tutoriels/954/notions-de-python-avancees/2-functions/1-callables/) pour en savoir plus.
...
Les _hooks_ sont appelables et attendent en argument un chemin, puis ils renvoient un objet _finder_.
...
Elle contient aussi un attribut `loader` associé au _loader_ qui saura charger un module à partir de ce fichier.  
...
- On construit les _finders_ à l'aide des listes `sys.path` et `sys.path_hooks` (la liste `path_finders` que l'on a construite plus tôt dans cette section).
...
![Diagramme de flux](img/simple_import_workflow.png)
...
```pycon
>>> my_import('functools')
<module 'functools' from '/usr/lib/python3.13/functools.py'>
```
...
[Le module `tarfile` de Python](https://docs.python.org/fr/3/library/tarfile.html) nous permet de manipuler les archives tar (et `.tar.gz` par extension).  
...
| On préférera donc `extractfile` qui nous renvoie un objet-fichier que l'on peut utiliser depuis l'interpréteur.
